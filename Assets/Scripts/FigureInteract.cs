...
public class FigureInteract : MonoBehaviour
{
    ...
    public virtual void Attack(FigureInteract enemy, List<string> flags)
    {
        ...
        else
        {
            ...
            TakeDamage(enemy.TotalDamage, true, false);
        }
    ...
    public virtual void TakeDamage(int damage, bool showNumbers = true, bool playSound = true)
    {
        if(damage != 0 && showNumbers) EffectsSystem.Instance.PlayDamage(transform.position, damage);
        ...
        if(playSound) SoundsSystem.Instance.Play(_damageSound, _damageSoundVolume);
    }
